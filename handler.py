...
def get_welcome_response(session):
    ...
    if collection_day is None:
        ...
        speech_output = "Your next bin collection is %s %s. " % (
            " and ".join(collection.types),
            collection.friendly_date,
        )
...
def set_user_collection_day(intent, session):
    ...
    if collection_day in ("MONDAY", "TUESDAY", "WEDNESDAY", "THURSDAY", "FRIDAY"):
        ...
        speech_output = (
            "I have stored your regular collection day as %s. You can now ask, when are the bins collected."
            % collection_day
        )
